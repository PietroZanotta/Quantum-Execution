...
def simulate_classical_circ(n_shots=100, n_iter=None, plot=True):
    ...
    three_or_four = QuantumRegister(digits)
    anc = QuantumRegister(2)
    one = QuantumRegister(digits)
    cx = ClassicalRegister(digits)

    qc = QuantumCircuit(x, three_or_four, one, anc)

    # init
    qc.h(three_or_four[2]) # 0100 + 0011
    qc.cx(three_or_four[2], three_or_four[0:2])
    qc.x(three_or_four[0:2])
    qc.h(x[0:3]) # 0abc

    # +3/4
    qc_ = QuantumCircuit(x, three_or_four)
    add(qc_, three_or_four, x, digits-1)
    qc_dagger = qc_.inverse()

    qc.append(qc_, list(x) + list(three_or_four))
    qc.cx(x[3], anc[0])
    qc.append(qc_dagger, list(x) + list(three_or_four))

    # controlled +1
    qc.x(one[0]) # 0001

    cadd(qc, anc[0], one, x, digits-1)
    ...
    fpqs_qc = fpqs_circ(oracle, .5, 4, A, 13, x, n_iter)
    # print(fpqs_qc)
...
# simulate_classical_circ(n_shots=100)
