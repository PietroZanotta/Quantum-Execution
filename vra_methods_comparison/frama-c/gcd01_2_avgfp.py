...
assert_line_index = next(
    i for i, line in enumerate(original_content) if "assert m == a || m == b" in line
)
...
for tuple_length in range(2, 8):
    ...
    for number_t in selected_number_tuples:
        for y_t in selected_y_tuples:
            ...
            number_condition = " || ".join([f"m == {x}" for x in number_t])
            y_condition = " || ".join([f"n == {x}" for x in y_t])
            ...
            for line in frama_output.splitlines():
                # if "result" in line:
                #     print(line)
                if ("2147483646" in line or "2147483647" in line) and "result ∈" in line:
            ...
            try:
                subprocess.run(
                    ["gcc", f"/home/pietro/Desktop/cu/vra_methods_comparison/frama-c/{str(file)}", "-o", "gcd"], text=True, capture_output=True
                )
            except Exception as e:
