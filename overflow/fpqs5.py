...
def s_alpha(alpha, num_qubits, A):
    ...
    circuit.rz(-alpha*0.5,num_qubits-5) 

    # Apply Multi-(XoX)-controlled NOT gate to last and ancilla qubit
    circuit.x(range(num_qubits-5))
    circuit.mcx(list(range(num_qubits-5)), num_qubits-5)
    circuit.mcx(list(range(num_qubits-5)), num_qubits)
    circuit.x(range(num_qubits-5))
