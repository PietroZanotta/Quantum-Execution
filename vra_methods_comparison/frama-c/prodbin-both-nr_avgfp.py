    ...
    for number_t in selected_number_tuples:
        for y_t in selected_y_tuples:
            ...
            if mask == 0:  # Extract closest values from Frama-C output
                closest_match = re.search(r"z ∈ (\{[0-9; ]+\}|\[[0-9.]+\])", frama_output)
                if closest_match:
            ...
            try:
                subprocess.run(
                    ["gcc", f"/home/pietro/Desktop/cu/vra_methods_comparison/frama-c/{str(file)}", "-o", "prod"], text=True, capture_output=True
                )
            except Exception as e:
