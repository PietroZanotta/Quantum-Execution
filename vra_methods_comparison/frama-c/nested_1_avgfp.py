...
for tuple_length in range(2, 8):
    ...
    for t in selected_tuples:
        ...
        tuple_condition = " || ".join([f"h == {x}" for x in t])
        ...
        for line in frama_output.splitlines():
            if ("2147483646" in line or "2147483647" in line) and "q ∈" in line:
        ...
        if mask == 0:
            ...
            closest_match = re.search(r"q ∈ (\{[0-9; ]+\}|\[[0-9.]+\])", frama_output)
            if closest_match:
        ...
        try:
            subprocess.run(
                ["gcc", f"/home/pietro/Desktop/cu/vra_methods_comparison/frama-c/{str(file)}", "-o", "n1"], text=True, capture_output=True
            )
        except Exception as e:
