...
def s_alpha(alpha, num_qubits, A, n_clbit):
    """
    ...
    circuit = QuantumCircuit(num_qubits + 1, n_clbit, name="S(\alpha)")

    # Apply A_dagger 
    circuit.append(A.inverse(), range(num_qubits+1), range(0,n_clbit))# !!!, range(0,6))        

    # Apply Z_{-alpha0.5} to last qubit (not ancilla qubit)
    # circuit.rz(-alpha*0.5,11) 
    circuit.barrier()
    circuit.rz(-alpha*0.5,num_qubits-2) 
    circuit.barrier()

    # Apply Multi-(XoX)-controlled NOT gate to last and ancilla qubit
    # circuit.x(range(9, num_qubits-9))
    # circuit.mcx(list(range(9, num_qubits-9)), 8)
    # circuit.mcx(list(range(9, num_qubits-9)), num_qubits)
    # circuit.x(range(9, num_qubits-11))
    ...
    # circuit.x(range(9, num_qubits-11))
    # circuit.mcx(list(range(9, num_qubits-11)), 8)
    # circuit.mcx(list(range(9, num_qubits-11)), num_qubits)
    # circuit.x(range(9, num_qubits-11))
    ...
    circuit.append(A, range(num_qubits+1), range(0,n_clbit)) 
    # print(circuit)  # !!!
    return circuit


def fpqs_circ(oracle, delta, n_qubits, A, n_clbit, num_steps=None):
    """
    ...
    print("num step" + str(num_steps))

    # Get the dimension of the circuit from A
    num_qubits = A.num_qubits - 1
    ...
    for j in range(num_steps):
        ...
        qc.compose(s_alpha(alpha, num_qubits, A, n_clbit), inplace=True)    # S(\alpha_j)
