...
tuples = list(itertools.permutations(range(n+1), 2))
...
for a, b in pairs:
    ...
    modified_line = f"    /*@ assert x == {a} || x == {b}; */\n"
    ...
    try:
        print(f"Running command for pair ({a}, {b}):")
        result = subprocess.run(
            terminal_command, shell=True, text=True, capture_output=True
        )
        frama_output = result.stdout
        print(f"Command Output for pair ({a}, {b}):")
        print(frama_output)
    except Exception as e:
        print(f"Error executing command for pair ({a}, {b}): {e}")
        continue
    ...
    if numbers_exceed_7:
        print(f"Error: Some values exceed 7 in results for ({a}, {b}).")
        break

    all_manual_in_frama = program_results <= frama_closest
    if not all_manual_in_frama:
        print(f"Error: Not all manual results are in Frama-C results for ({a}, {b}).")
        break
