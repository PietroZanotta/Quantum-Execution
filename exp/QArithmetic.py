...
def cadd(circ, c, a, b, n):
    ...
    for i in range(n, 0, -1):
        for j in range(i, 0, -1):
            if len(a) - 1 >= j - 1:
                circ.mcp(2*pi/2**(i-j+1), [c, a[j-1]], b[i-1])  # Replaces ccu1
    # circ.barrier()
    ciqft(circ, c, b, n)

# Adder that takes |a>|b> to |a>|a+b> using ripple-carry.
...
def cmult(circ, control, a, b, c, n):
    ...
    tempCircuit = tempCircuit.control(1)
    circ.compose(tempCircuit, qubits=control[:] + a[:] + b[:] + c[:], inplace=True)
