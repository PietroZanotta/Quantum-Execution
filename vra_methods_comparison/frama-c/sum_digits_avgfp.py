...
for tuple_length in range(2, 8):
    ...
    for t in selected_tuples:
        ...
        if mask == 0:  # Extract closest values from Frama-C output
            ...
            closest_match = re.search(r"digit_sum ∈ (\{[0-9; ]+\}|\[[0-9.]+\])", frama_output)
            if closest_match:
        ...
        try:
            subprocess.run(
                ["gcc", f"/home/pietro/Desktop/cu/vra_methods_comparison/frama-c/{str(file)}", "-o", "sum"], text=True, capture_output=True
            )
        except Exception as e:
