...
for tuple_length in range(2, 8):
    ...
    for number_t in selected_number_tuples:
        for y_t in selected_y_tuples:
            ...
            number_condition = " || ".join([f"x == {x}" for x in number_t])
            y_condition = " || ".join([f"y == {x}" for x in y_t])
            ...
            try:
                subprocess.run(
                    ["gcc", f"/home/pietro/Desktop/cu/vra_methods_comparison/frama-c/{str(file)}", "-o", "fs"], text=True, capture_output=True
                )
            except Exception as e:
