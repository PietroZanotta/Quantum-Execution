...
assert_line_index = next(
    i for i, line in enumerate(original_content) if "assert A == a || A == b" in line
)
...
for tuple_length in range(2, 8):
    ...
    for number_t in selected_number_tuples:
        for y_t in selected_y_tuples:
            ...
            number_condition = " || ".join([f"A == {x}" for x in number_t])
            y_condition = " || ".join([f"q == {x}" for x in y_t])
