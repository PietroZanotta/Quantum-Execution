...
assert_line_index = next(
    i for i, line in enumerate(original_content) if "assert n == a || n == b" in line
)
...
for tuple_length in range(2, 8):
    ...
    for t in selected_tuples:
        ...
        tuple_condition = " || ".join([f"n == {x}" for x in t])
        ...
        for line in frama_output.splitlines():
            if "2147483647" in line and "fact ∈" in line:
        ...
        if mask == 0:
            ...
            closest_match = re.search(r"fact ∈ (\{[0-9; ]+\}|\[[0-9.]+\])", frama_output)
            if closest_match:
        ...
        try:
            subprocess.run(
                ["gcc", f"/home/pietro/Desktop/cu/vra_methods_comparison/frama-c/{str(file)}", "-o", "fact"], text=True, capture_output=True
            )
        except Exception as e:
