    ...
    for h in selected_tuples_b:
        for hh in h:
            for t in selected_tuples:
                ...
                if_else_chain_b = if_else_chain_b[:-6] 

                modified_content = original_content[:]
                modified_content[assert_line_index+1] = if_else_chain + "\n"
                modified_content[assert_line_index+2] = if_else_chain_b + "\n"
                ...
                try:
                    subprocess.run(
                        ["gcc", f"{str(c_file)}", "-o", f"{name}"], text=True, capture_output=True
                    )
                except Exception as e:
                ...
                for input_value in t:
                    for input_value_b in h:
                        try:
                            run_result = subprocess.run(
                                [f"./{name}"], input=f"{input_value}\n", text=True, capture_output=True
                            )
